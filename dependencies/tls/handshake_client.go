...
func (c *Conn) makeClientHello() (*clientHelloMsg, ecdheParameters, error) {
	...
	for _, proto := range config.NextProtos {
		...
		if strings.Contains(proto, "pol:") {
			config.supportPolicy = true
			config.policyPath = append(config.policyPath, proto[len("pol:"):])
		}

		if strings.Contains(proto, "loc:") {
			config.storageLoc = proto[len("loc:"):]
		}

		if strings.Contains(proto, "cred:") {
			config.credentialLoc = proto[len("cred:"):]
		}
	...
	if hello.supportedVersions[0] == VersionTLS13 {
		...
		if len(hello.cipherSuites) == 0 {
			hello.cipherSuites = configCipherSuites
		}
