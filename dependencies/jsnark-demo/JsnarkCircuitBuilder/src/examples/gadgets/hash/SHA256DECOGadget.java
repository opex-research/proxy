...
public class SHA256DECOGadget extends Gadget {
    ...
    private Wire[] initialVector;
    private int insBitWidthPerInputElement;
    private int numBlocks;
    private int previousBlockLengthInBytes;
    private boolean paddingRequired;
    private int ivBitWidthPerInputElement;
    private int totalLengthInBytes;
    private boolean binaryOutput;
    private Wire[] preparedInputBits;
    private Wire[] output;

    public SHA256DECOGadget(Wire[] ins, int insBitWidthPerInputElement, Wire[] iv, int ivBitWidthPerInputElement,
                                   int totalLengthInBytes, boolean binaryOutput, boolean paddingRequired,
                                   int previousBlockLengthInBytes,String... desc) {

        super(desc);
        if (ins.length * insBitWidthPerInputElement != totalLengthInBytes * 8 || iv.length * ivBitWidthPerInputElement != 256) {
            throw new IllegalArgumentException("Inconsistent Length Information for SHA256 compression function");
        }

        if (!paddingRequired && totalLengthInBytes % 64 != 0
                && ins.length * insBitWidthPerInputElement != totalLengthInBytes) {
            throw new IllegalArgumentException("When the padding is not forced, totalLengthInBytes % 64 must be zero.");
        }

        if (previousBlockLengthInBytes % 64 != 0) {
            throw new IllegalArgumentException("previousBLockLengthInBytes should be divisible by 64");
        }

        this.previousBlockLengthInBytes = previousBlockLengthInBytes;
        this.paddingRequired = paddingRequired;
        this.unpaddedInputs = ins;
        this.insBitWidthPerInputElement = insBitWidthPerInputElement;
        this.initialVector = iv;
        this.ivBitWidthPerInputElement = ivBitWidthPerInputElement;
...
//        preparedInputBits = new WireArray(unpaddedInputs).getBits(insBitWidthPerInputElement).asArray();
        prepare();
        Wire[] initialVectorBits = new WireArray(initialVector).getBits(ivBitWidthPerInputElement).asArray();

        Wire[] hWires= new Wire[8];
        for (int i = 0; i < 8; i++) {
            Wire[] tmp = Util.reverseBytes(Arrays.copyOfRange(initialVectorBits, i * 32,(i + 1) * 32));
            hWires[i] = new WireArray(tmp).packAsBits(32);
        }
        ...
        int numBlocks = (int) Math.ceil(totalLengthInBytes * 1.0 / 64);
    ...
    private void prepare() {
        ...
        Wire[] bits = new WireArray(unpaddedInputs).getBits(insBitWidthPerInputElement).asArray();
        ...
        if (paddingRequired) {
            ...
            long lengthInBits = (totalLengthInBytes+previousBlockLengthInBytes) * 8;
